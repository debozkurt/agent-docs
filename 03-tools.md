...
Use the property deliberately: each agent should bind exactly the tools it needs for its job — no more. The principle is **minimum necessary tools**, not a rigid read/write split. A task agent often needs both read tools (to look up current state) and write tools (to make changes) — that's fine, as long as it doesn't *also* have tools for unrelated domains. The worked example in this guide binds all four todo tools (list, create, complete, update) to a single agent because they all serve the same job.

Where the read/write distinction *does* matter is at the agent boundary when you've split into specialists (Chapter 13–14). A query-only agent that answers questions should not have write tools bound — not because reads and writes can't coexist, but because that agent's job doesn't require mutations. A task agent that creates and completes items absolutely should have both reads and writes. The question isn't "read or write?" — it's "what does this agent's job require?"

The discipline this enables is the **negative assertion in tests**: not "this handler calls the right tool" but "this handler must NOT have tools outside its domain." A one-line test (`assert UNRELATED_TOOLS.isdisjoint(agent.bound_tools)`) is the strongest safety check available, because it doesn't depend on the model behaving well at runtime — it catches a future refactor that accidentally widens the bound set, or a copy-paste error that gives an agent tools it was never meant to have.
